...
eval "mvn package appengine:deploy clean -Dapp.deploy.version=$versionName -Dapp.deploy.promote=False"

# Run end-to-end tests.
failUrls=0
successUrls=0
for script in end-to-end-tests/*.sh; do
  if ! sh $script "http://$versionName-dot.photoblur.ew.r.appspot.com/" -H; 
  then 
...
eval "gcloud app versions delete $versionName --quiet"
...
if [ $failUrls = 0 ];
  ...
  eval "mvn package appengine:deploy clean -Dapp.deploy.version=1"
fi
