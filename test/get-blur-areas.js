...
describe('Connection with server part', () => {
  ...
  describe('getFormUploadUrl()', () => {
    it('check that it fetchs expected url using get method and gets response from it',
        async() => {
  ...
  describe('getBlurAreas()', () => {
    ...
    it('invalid blobstore upload url', () => {
      return new Promise(function(resolve, reject) {
        const FETCH_URL_TO_BLOBSTORE = '/blobstore-upload-url?forwardurl=/get-blur-areas';
        const STATUS_CODE = 400;
    ...
    it('normal response with one valid rect', () => {
      return new Promise(function(resolve, reject) {
        ...
        const RESPONSE = [[{'x': 10, 'y': 10}, {'x': 20, 'y': 10}, {'x': 20, 'y': 20}, {'x': 10, 'y': 20}]];
  ...
  describe('Rect constructor', () => {
    it('not a rectangle - some other polygon with 4 points', () => {
      const rect = [{'x': 10, 'y': 10}, {'x': 10, 'y': 20}, {'x': 20, 'y': 10}, {'x': 20, 'y': 111}];
    ...
    it('rect is greater than image size', () => {
      const rect = [{'x': 10, 'y': 10}, {'x': 10, 'y': 20}, {'x': 100, 'y': 10}, {'x': 100, 'y': 20}];
      const image = new Image(10, 200);

      expect(() => new Rect(rect, image)).to.throw('Rect is greater than image');
    });

    it('rect has negative top left point', () => {
      const rect = [{'x': -10, 'y': 10}, {'x': -10, 'y': 20}, {'x': 20, 'y': 10}, {'x': 20, 'y': 20}];
      const image = new Image();
